...
func TestListWorkers(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "filter by pool_id",
			queryParams:    "?pool_id=" + pool1.ID.String(),
			expectedStatus: http.StatusOK,
			minWorkers:     2,
			maxWorkers:     2,
		},
		...
		},
		{
			...
			queryParams:    "?online=false",
			expectedStatus: http.StatusOK,
			minWorkers:     0,
			maxWorkers:     0,
		},
		{
			name:           "invalid pool_id",
			queryParams:    "?pool_id=invalid-uuid",
			expectedStatus: http.StatusOK,
			minWorkers:     3,
			maxWorkers:     3,
		},
		{
			name:           "invalid online parameter",
			queryParams:    "?online=invalid",
			expectedStatus: http.StatusOK,
			minWorkers:     3,
			maxWorkers:     3,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req, err := http.NewRequest("GET", "/api/v1/workers"+tt.queryParams, nil)
			require.NoError(t, err)
			...
			if tt.expectedStatus == http.StatusOK {
				...
				assert.GreaterOrEqual(t, len(workers), tt.minWorkers)
				assert.LessOrEqual(t, len(workers), tt.maxWorkers)
...
func TestGetWorker(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "get worker with invalid UUID",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req, err := http.NewRequest("GET", "/api/v1/workers/"+tt.workerID, nil)
			require.NoError(t, err)
			...
			if tt.expectedStatus == http.StatusOK {
				...
				err := json.Unmarshal(rr.Body.Bytes(), &response)
				require.NoError(t, err)
				assert.Equal(t, testWorker.ID, response.ID)
				assert.Equal(t, testWorker.Name, response.Name)
			}
		})
	}
}
...
func TestWorkerHeartbeat(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "send heartbeat successfully",
			workerID: testWorker.ID.String(),
			requestBody: map[string]interface{}{
				"active": 3,
			},
		...
		{
			name:     "send heartbeat with negative active count",
			workerID: testWorker.ID.String(),
			requestBody: map[string]interface{}{
				"active": -1,
			},
		...
		{
			name:           "send heartbeat with invalid JSON",
			workerID:       testWorker.ID.String(),
			requestBody:    "invalid json",
			expectedStatus: http.StatusBadRequest,
		},
		{
			name:     "send heartbeat to non-existent worker",
			workerID: uuid.New().String(),
			requestBody: map[string]interface{}{
				"active": 1,
			},
		...
		{
			name:     "send heartbeat with invalid UUID",
			workerID: "invalid-uuid",
			requestBody: map[string]interface{}{
				"active": 1,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := http.NewRequest("POST", "/api/v1/workers/"+tt.workerID+"/heartbeat", bytes.NewBuffer(body))
			require.NoError(t, err)
			req.Header.Set("Content-Type", "application/json")
			...
			if tt.expectedStatus == http.StatusOK {
				if tt.workerID == testWorker.ID.String() {
					updatedWorker, err := store.GetWorker(ctx, testWorker.ID)
					require.NoError(t, err)

					if originalBody, ok := tt.requestBody.(map[string]interface{}); ok {
						if active, exists := originalBody["active"]; exists {
							var expectedActive int
							switch v := active.(type) {
							case int:
								expectedActive = v
							case float64:
								expectedActive = int(v)
							}
							assert.Equal(t, expectedActive, updatedWorker.Active)
						}
					}
				}
			}
...
func TestPauseWorker(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "pause worker successfully",
			workerID: testWorker.ID.String(),
			requestBody: map[string]interface{}{
				"paused": true,
			},
		...
		{
			name:     "resume worker successfully",
			workerID: testWorker.ID.String(),
			requestBody: map[string]interface{}{
				"paused": false,
			},
		...
		{
			name:           "pause worker with invalid JSON",
			workerID:       testWorker.ID.String(),
			requestBody:    "invalid json",
			expectedStatus: http.StatusBadRequest,
		},
		...
		{
			name:     "pause worker with invalid UUID",
			workerID: "invalid-uuid",
			requestBody: map[string]interface{}{
				"paused": true,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := http.NewRequest("POST", "/api/v1/workers/"+tt.workerID+"/pause", bytes.NewBuffer(body))
			require.NoError(t, err)
			req.Header.Set("Content-Type", "application/json")
			...
			if tt.expectedStatus == http.StatusOK && tt.workerID == testWorker.ID.String() {
				updatedWorker, err := store.GetWorker(ctx, testWorker.ID)
				require.NoError(t, err)
				assert.Equal(t, tt.expectedPaused, updatedWorker.Paused)
			}
		})
	}
}
...
func TestDeleteWorker(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "delete non-existent worker",
			workerID:       uuid.New().String(),
			expectedStatus: http.StatusOK,
		},
		{
			name:           "delete worker with invalid UUID",
			workerID:       "invalid-uuid",
			expectedStatus: http.StatusBadRequest,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req, err := http.NewRequest("DELETE", "/api/v1/workers/"+tt.workerID, nil)
			require.NoError(t, err)
