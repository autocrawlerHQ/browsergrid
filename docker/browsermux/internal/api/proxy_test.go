...
func TestCDPReverseProxyURLRewriting(t *testing.T) {
	// Mock backend server that returns JSON with internal port
	...
	// Create a browser URL that uses port 61000 to simulate the real scenario
	...
	// Override the proxy to use our test backend
	...
	// Create a request with external port 32771
...
func TestCDPReverseProxyNonJSONResponse(t *testing.T) {
	// Mock backend server that returns non-JSON content
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	// Non-JSON content should NOT be rewritten
	if !strings.Contains(body, ":61000") {
