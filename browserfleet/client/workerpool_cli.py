#!/usr/bin/env python
"""
BrowserFleet Work Pool CLI

Command-line tool for creating and managing BrowserFleet work pools.
"""
import os
import sys
import argparse
...
class WorkPoolCLI:
    ...
    def start_worker(self, pool_id: str, worker_name: Optional[str] = None) -> None:
        ...
        command = [
            f"{python_executable} -m browserfleet.client.worker",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="BrowserFleet Work Pool Management CLI"
    )
    ...
    parser.add_argument(
        "--api-url",
        required=True,
        help="BrowserFleet API URL",
        default=os.environ.get("BROWSERFLEET_API_URL")
    )
    parser.add_argument(
        "--api-key",
        required=True,
        help="BrowserFleet API key",
        default=os.environ.get("BROWSERFLEET_API_KEY")
    )
    ...
    if args.command == "create":
        ...
        print(f"python -m browserfleet.client.workerpool_cli start-worker {result['id']}")
