"""
BrowserFleet Worker Client

This script runs on worker nodes to poll for sessions from a BrowserFleet server.
...
logger = logging.getLogger("browserfleet-worker")

# Import enums
try:
    from browserfleet.server.sessions.enums import SessionEventType, SessionStatus
except ImportError:
...
class BrowserFleetWorker:
    """Client for BrowserFleet worker node"""
    ...
    def _make_api_request(self, method: str, endpoint: str, json_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Make an API request to the BrowserFleet server"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BrowserFleet Infrastructure-Agnostic Worker Client")
    parser.add_argument("--api-url", required=True, help="BrowserFleet API URL")
    parser.add_argument("--api-key", required=True, help="BrowserFleet API key")
